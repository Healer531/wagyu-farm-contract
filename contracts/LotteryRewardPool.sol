...
contract LotteryRewardPool is Ownable {
    ...
    constructor(MasterChef _chef, IBEP20 _wag, address _admin, address _receiver) {
        chef = _chef;
        wag = _wag;
        adminAddress = _admin;
        receiver = _receiver;
    ...
    function  harvest(uint256 _pid) external onlyAdmin {
        chef.deposit(_pid, 0);
        uint256 balance = wag.balanceOf(address(this));
        wag.safeTransfer(receiver, balance);
    ...
    function  pendingReward(uint256 _pid) external view returns (uint256) {
        return chef.pendingWag(_pid, address(this));
    }

    // EMERGENCY ONLY.
    function emergencyWithdraw(uint256 _amount) external onlyOwner {
        wag.safeTransfer(address(msg.sender), _amount);
